...
In addition to the above query helpers, you can also create custom Record queries using [`$app.recordQuery(collection)`](/jsvm/functions/_app.recordQuery.html) method. It returns a SELECT DB builder that can be used with the same methods described in the [Database guide](./js-database.md).
...
[Prev: Database](./js-database.md) [Next: Collection operations](./js-collections.md)